<x-app-layout>
    <div class="py-8 px-4 sm:px-6 lg:px-8">
        <div class="max-w-7xl mx-auto space-y-6">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-cyan-100 text-sm">
                                Actualiza los resultados del paciente
                            </p>
                        </div>
                    </div>
            ...
            <form
                action="{{ route('examenes.update', $consulta->id) }}"
                method="POST"
                class="space-y-6"
            >
                @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input
                                type="text"
                                name="aspecto"
                                value="{{ old('aspecto', $primero->aspecto ?? '') }}"
                                class="w-full rounded-xl border-gray-300 shadow-sm focus:ring-cyan-500 focus:border-cyan-500"
                            >
                        </div>
                    </div>
                        ...
                        <table class="min-w-full text-sm">
                            ...
                            <tbody class="divide-y divide-gray-100 bg-white">
                                @foreach ($parametros as $key => $item)
                                    @php
                                        $valor = $examenes->firstWhere('parametro', $item['label']);
                                    @endphp
                                    ...
                                    <tr class="hover:bg-cyan-50/60 transition-colors duration-200">
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            <input
                                                type="text"
                                                name="examen[{{ $key }}][resultado]"
                                                value="{{ old("examen.$key.resultado", $valor->resultado ?? '') }}"
                                                class="w-32 text-center rounded-full border-gray-300 shadow-sm focus:ring-cyan-500 focus:border-cyan-500"
                                            >

                                            <input
                                                type="hidden"
                                                name="examen[{{ $key }}][parametro]"
                                                value="{{ $item['label'] }}"
                                            >

                                            <input
                                                type="hidden"
                                                name="examen[{{ $key }}][referencia_perro]"
                                                value="{{ $item['perro'] }}"
                                            >

                                            <input
                                                type="hidden"
                                                name="examen[{{ $key }}][referencia_gato]"
                                                value="{{ $item['gato'] }}"
                                            >
                                        </td>
